...
- [ ] `features/notifications/components/form/SendNotificationModal.tsx` — no-adjust-state×8, prefer-useReducer×1, no-cascading-set-state×1, no-reset-all-state×1, no-event-handler×1 _(auto-preview already done)_
...
- [ ] `components/ui/filters/ClientPickerFilter.tsx` — no-derived-state×1, no-event-handler×2 _(shared filter)_
- [ ] `components/shared/client/ClientSearchInput.tsx` — prefer-useReducer×1, advanced-event-handler-refs×1 _(+1 exhaustive-deps FP)_
- [ ] `hooks/useSearchDebounce.ts` — no-derived-state×1, no-derived-state-effect×1 _(consumed widely)_
