 ...
 * readable.
 *
 * Links to the payment it names, which is what makes "readable" true — a
 * superseded payment is fetchable by id precisely so a corrected period's
 * history can be opened. Two rows are deliberately not links: the payment
      ...
      {isLoading ? (
        ...
        <ol className="space-y-2">
          {data.map((record) => {
            ...
            return (
              <li key={record.id}>
                {record.is_withdrawn || record.id === paymentId ? (
