...
export const vatWorkItemCreateSchema = z.object({
  client_id: z
    .string()
    .trim()
    .min(1, VAT_ERROR_MESSAGES.validation.clientRequired)
    .refine((v) => Number.isInteger(Number(v)) && Number(v) > 0, {
      message: VAT_ERROR_MESSAGES.validation.validClientRequired,
    }),
  period: z
    .string()
    .trim()
    .min(1, VAT_ERROR_MESSAGES.validation.periodRequired)
    .refine((v) => PERIOD_PATTERN.test(v), {
      message: VAT_ERROR_MESSAGES.validation.periodFormat,
    }),
