...
const isPercentField = (field: FieldDef): boolean =>
  field.key.includes('rate') || field.key.includes('percentage') || field.label.includes('%') || field.label.includes('שיעור')
...
const isMoneyField = (field: FieldDef): boolean => field.type === 'number' && !isPercentField(field) && !isCountField(field)
...
export const getCompletedCount = (schedules: ScheduleEntry[]) => schedules.filter((schedule) => schedule.is_complete).length
