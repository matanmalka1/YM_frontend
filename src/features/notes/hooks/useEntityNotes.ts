...
export type NotesTarget = { scope: 'client'; clientId: number } | { scope: 'business'; clientId: number; businessId: number }
  ...
  const options =
    target.scope === 'business'
      ? {
          ...
          update: (noteId: number, note: string) => notesApi.updateForBusiness(clientId, target.businessId, noteId, { note }),
