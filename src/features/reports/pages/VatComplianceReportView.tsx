...
const buildComplianceColumns = (year?: number): Column<VatComplianceItem>[] => [
  ...
  {
    key: 'expected',
    header: REPORTS_MESSAGES.vatCompliance.expectedPeriods,
    render: (item) => numericCell(item.periods_expected),
  },
  ...
  { key: 'late', header: REPORTS_MESSAGES.vatCompliance.late, render: (item) => numericCell(item.late_count) },
  {
    ...
    render: (item) => <Badge variant={complianceBadgeVariant(item.compliance_rate)}>{formatPercent(item.compliance_rate)}</Badge>,
  },
]
...
const StalePendingTable = ({ items }: { items: StalePendingItem[] }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <DataTable data={items} columns={STALE_PENDING_COLUMNS} getRowKey={(item) => `${item.client_record_id}-${item.period}`} />
    </div>
  )
}
    ...
    <PageStateGuard isLoading={isLoading} error={error} header={header} loadingMessage={REPORTS_MESSAGES.common.loadingReport}>
      {data && (
        <PageContent>
          ...
          {totalPages > 1 && <PaginationCard page={page} totalPages={totalPages} total={data.total} onPageChange={setPage} />}
